...
Views.dashboard = (function () {
  ...
  function buildCalendar(year, month, selectedDate) {
    ...
    for (var i = 0; i < 42; i++) {
      ...
      if (hwOnDate.length) markers += '<span style="font-size:9px">📝</span>';
      if (libOnDate.length) markers += '<span style="font-size:9px">📖</span>';
      if (dayOverview.expected.length) markers += '<span style="font-size:9px">👥</span>';

      html += '<div class="' + classes + '" data-date="' + dateStr + '">' +
              date.getDate() +
              (markers ? '<div style="font-size:8px;letter-spacing:-1px;margin-top:1px">' + markers + '</div>' : '') +
              '</div>';
  ...
  function render(el) {
    ...
    el.innerHTML =
      ...
      // 상단 미니 캘린더 + 이번주 예정
      '<div style="background:linear-gradient(135deg,#003d99 0%,#0052cc 100%);border-radius:12px;padding:20px;color:#fff;box-shadow:0 2px 8px rgba(0,61,153,.15);margin-bottom:24px">' +
      ...
      // 민감한 정보 (하단)
      '<div class="section-title" style="margin-top:32px;margin-bottom:16px">📊 관리 현황 요약</div>' +
      sensitiveCards() +
